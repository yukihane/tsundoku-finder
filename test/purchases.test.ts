...
function fixture({
    ...
    ${[1, duplicate ? 1 : 2]
			.map(
				(n) => `<div id="content-title-B00000000${n}">架空の本 ${n}</div>
      <div id="content-author-B00000000${n}">架空の著者</div>
      ${missingDate ? "" : `<div id="content-acquired-date-B00000000${n}">取得日: 2026年9月1日</div>`}`,
			)
			.join("")}`;
}
...
test("purchased classification requires matching URL, selected filter and complete rows", async () => {
	...
	try {
		...
		for (const url of [
			purchasesUrl.replace("booksPurchases", "kuAll"),
			`${purchasesUrl}?pageNumber=2`,
			`${purchasesUrl}?search=test`,
...
test("purchase limit matches a single page", () => {
	for (const limit of [0, -1, 26, 1.5, NaN])
		assert.throws(() => validatePurchaseLimit(limit));
