...
## 11. 将来のゲーム対応を見据えた設計候補

検討日: 2026-09-22。将来のゲーム対応という要望に対する設計案。初期版は引き続きKindleの書籍のみを対象とし、以下の詳細は採用確定ではない。

### 今から分けておく境界

- **共通基盤と種類固有の処理**: 同期の実行・記録、再試行、出典管理、DBマイグレーション、MCP接続などは共通化の候補。書籍のISBN・著者・巻・版や、ゲームの対応環境・開発元・DLCなどの情報と判定処理は種類別に保持する。共通の型でISBNなどを書籍固有の必須項目にしない。
- **ストアとコンテンツの種類**: ストア名だけで書籍かゲームかを決めない。取得元サービスと種類を別々に表現する。外部IDは提供元・サービスで名前空間を分け、内部IDとは別に保持する。
- **商品と所有・利用情報**: 商品の説明と、どのアカウントがどこで利用できるかを分ける。ライブラリに表示されたことだけで有料購入や永続所有と断定しない。取得できない購入日時・金額・権利種別などは不明として扱えるようにする。
- **取得方式と同期処理**: 各取得処理はAPIやブラウザの詳細を内部に隠し、共通形式の結果と取得の完了・失敗状況を返す。共通インターフェースでPlaywrightのブラウザオブジェクトを必須にしない。所有情報の取得元と説明情報の補完元も分ける。
- **検索基盤と検索条件**: タイトル・紹介文の検索、件数制限、出典の返却は共通化の候補。著者・巻といった書籍向け条件や、プレイ人数などのゲーム向け条件は種類別に追加できる構造にする。

データの考え方は「共通項目（内部ID・種類・タイトルなど）＋書籍固有情報」「提供元ごとの商品ID・URL」「アカウントの所有・利用情報」「出典付き補足情報」に分ける案。具体的なテーブル構造やゲーム用スキーマは今決めず、書籍とゲームの全項目を一つの巨大な型に詰め込まない。

### ゲーム対応時に判断すること

- 本編・DLC・体験版・セット商品の扱いと、セットから個別作品への対応付け。
- 無料取得、期限付き利用、共有などを推薦対象に含める範囲と、取得元から判別可能な情報。
- 別ストア版・リマスター版などを同じ作品として関連付ける基準。名前の一致だけで統合しない。
- プレイ時間・クリア状況・既読状態などの進捗管理を行うか。所有していることと未読・未プレイであることは別であり、自動で推定しない。
- 書籍・ゲームを同じDBとMCPで扱うか、共通基盤を使った別アプリとして提供するか。現段階では分離可能なモジュール構造にとどめる。

### 初期版での実装範囲

まずKindleの取得処理をモジュールとして登録する方式とし、公開プラグインSDK・動的なプラグイン配布・ゲーム用の空テーブルなどは先行実装しない。2つ目の実例ができた段階で共通化の範囲を見直す。

MCPは初期版の `search_books` などを維持してもよい。将来は共通の検索処理を使ってゲーム用ツールを追加できるため、現在の外部インターフェースまで曖昧な汎用名にする必要はない。
