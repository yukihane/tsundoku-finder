...
- コンテンツ管理画面の「本 → 購入済み」フィルターと、独立したKindle Unlimited・Prime Readingカテゴリーを確認した。次は購入済み一覧の少数取得を専用Chromiumで検証する案を推奨する。詳細は調査記録の追加調査を参照。現在の本棚由来の`ownership: unknown`を購入済みとしてDB登録しない。
- 所有区分には判定根拠（情報源・選択したフィルター・取得日時）を関連付ける案を検討する。有料購入と無料取得の区別は未検証であり、購入済み一覧への掲載から金額を推測しない。
- 本棚の追加読み込み、重複排除、終了条件、途中失敗を設計する。カード数の一時的な停止だけで全件取得を成功扱いしない。
- ログイン状態の再利用・期限切れ、空本棚、検索や絞り込み中、DOM変更を区別する。
- 利用条件とアクセス制限の確認を継続する。現段階では連続巡回や自動再試行を実装していない。
