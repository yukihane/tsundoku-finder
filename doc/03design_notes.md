...
1. KindleはAmazon.co.jpの本棚DOMから少数取得するプロトタイプを作成済み。全件取得と購入済みの判定方法を引き続き確認する。O'Reillyなど他ストアの詳細確認は各ストアの対応時に行う。
...
### 2026-09-22のプロトタイプ後に残る検証

- 専用ブラウザから10件のJSON保存まで確認済み。詳細な観測結果は[調査記録](02product_evaluation.md#8-kindle-for-webの取得プロトタイプ)を参照。
- コンテンツ管理画面の「本 → 購入済み」から10件の取得を専用Chromiumで検証済み。判定根拠（情報源・選択したフィルター・取得日時）を保存する。現在の本棚由来の`ownership: unknown`を購入済みとしてDB登録しない。
- 購入済み一覧のページ取得・件数照合・途中保存・中断処理を実装し、実アカウントで全191ページ・4,769件の取得と保存JSONの照合を完了した。途中再開、空一覧の識別は今後の課題。有料購入と無料取得の区別は未検証であり、購入済み一覧への掲載から金額を推測しない。
- 次の実装候補は、取得済みJSONを入力とするSQLiteへの登録。ストア商品IDで再取り込みの重複を防ぎ、取得根拠を保持し、未取得を理由に削除しない登録処理を先に固める。テーブル構成と取り込みCLIは別途設計する。
- 本棚の追加読み込み、重複排除、終了条件、途中失敗を設計する。カード数の一時的な停止だけで全件取得を成功扱いしない。
- ログイン状態の再利用・期限切れ、空本棚、検索や絞り込み中、DOM変更を区別する。
- 利用条件とアクセス制限の確認を継続する。全件取得はページ保存後3秒待つ逐次巡回とし、自動再試行は実装していない。
