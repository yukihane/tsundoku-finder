...
未検証: 空の購入済み一覧、長期的な認証維持、OSによる強制終了時の保存結果。途中再開は未実装。全件判定は取得中の件数と一意性の整合に基づき、ストア側の同時変更を完全に検出する保証はない。

### 全ページ通しの実アカウント検証

検証日: 2026-09-22。`pnpm dev kindle purchases --all`を実行し、191ページ・4,769件を約14分14秒で取得した。最終ページの19件まで保存し、`status: completed`、`complete: true`、終了コード0で終了した。今回の実行中、追加認証・取得エラー・自動再試行は発生しなかった。実装コードの修正は不要だった。

完了後に全ページのJSONを読み直し、以下を独立に照合した。

- レポートの総件数・取得件数・保存された書籍件数・一意なASIN件数が一致する。
- ページ番号と表示範囲が連続し、最終範囲が総件数に一致する。
- 全ページで購入済みフィルターの根拠、出典、取得日時がレポートと一致する。
- 書名・取得日の必須項目、ASINの形式、各書籍の所有区分が妥当である。
- ページファイル数がレポートのページ数に一致し、保存先がGit対象外である。

書名・ASIN・個々の取得日・認証情報・取得JSONはリポジトリに含めない。この検証は一つの実アカウントでの今回の成功を示すもので、空一覧や返金、別アカウント、取得中の同数の入れ替えを検証したものではない。
