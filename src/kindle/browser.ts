...
export async function openKindleBrowser(): Promise<void> {
	...
	try {
		...
		console.log(
			"専用ブラウザでAmazon.co.jpにログインしてください。追加認証もブラウザで行ってください。",
		);
		console.log("確認が終わったらブラウザのウィンドウを閉じてください。");
		console.log(
			"認証状態は .local/kindle/browser-profile/ に保存されます。通常のChromeプロファイルとは別です。",
		);
		await closed;
		console.log(
			"ブラウザを終了しました。ログイン成否や所有書籍の取得はまだ自動判定していません。",
		);
	} finally {
