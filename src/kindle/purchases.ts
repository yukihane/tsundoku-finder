...
export async function readPurchasePage(page: Page, limit: number) {
	...
	const snapshot = await page.evaluate(() => ({
		url: location.href,
		category: document
			.querySelector("#ContentCategoryDropDown .drop-down-text")
			?.textContent?.trim(),
		filter: document
			.querySelector("#ContentSubCategoryDropDown .drop-down-text")
			?.textContent?.trim(),
		search: (
			document.querySelector("#content-search-box") as HTMLInputElement | null
		)?.value,
		count: document.querySelector("#CONTENT_COUNT")?.textContent ?? "",
		rows: Array.from(document.querySelectorAll('[id^="content-title-"]')).map(
			(element) => {
				...
				return {
					...
					authorsText:
						document
							.getElementById(`content-author-${asin}`)
							?.textContent?.trim() ?? "",
					acquiredDateText:
						document
							.getElementById(`content-acquired-date-${asin}`)
							?.textContent?.trim() ?? "",
				};
			},
		),
	...
	if (
		...
		throw new Error(
			"Purchased page is incomplete or its structure has changed",
		);
	}
	...
	return {
		...
		books: snapshot.rows.slice(0, limit).map((row) => ({
			...row,
			ownership: "purchased",
			productUrl: `https://www.amazon.co.jp/dp/${row.asin}`,
...
export async function capturePurchasedSample(limit: number): Promise<void> {
	...
	try {
		...
		console.log(
			"購入済み一覧の表示を最大5分待ちます。ログイン画面が出た場合は手動で認証してください。",
		);
		...
		await page.waitForFunction(
			() => {
				const text =
					document.querySelector("#CONTENT_COUNT")?.textContent ?? "";
