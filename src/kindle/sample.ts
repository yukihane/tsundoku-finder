...
import { normalizeCards, type KindleCard } from './extract.js';

export async function readLibraryCards(page: Page): Promise<KindleCard[]> {
  const url = new URL(page.url());
  if (url.origin !== 'https://read.amazon.co.jp' || url.pathname !== '/kindle-library') {
  ...
  await page.locator('#cover [id^="library-item-option-"]').first().waitFor({ timeout: 30_000 });
  return page.locator('#cover [id^="library-item-option-"]').evaluateAll(elements =>
    elements.map(element => ({
      id: element.id,
      title: element.querySelector('[id^="title-"]')?.textContent ?? '',
      authorsText: element.querySelector('[id^="author-"]')?.textContent ?? '',
      badges: Array.from(element.querySelectorAll('[id^="plansBadge-"], [id^="canvasBadge-"], [id^="badge-"]'))
        .map(badge => badge.getAttribute('aria-label') ?? badge.textContent ?? '')
        .filter(Boolean),
    })),
  );
}
...
export async function captureKindleSample(limit: number): Promise<void> {
  ...
  try {
    ...
    console.log('専用ブラウザでログインしてください。本棚の表示を最大5分待ちます。');
    await page.locator('#cover [id^="library-item-option-"]').first().waitFor({ timeout: 300_000 });
    ...
    const directory = fileURLToPath(new URL('../../.local/kindle/samples/', import.meta.url));
    ...
    console.log(`${books.length}件を .local/kindle/samples/${filename} に保存しました。`);
    console.log('表示済みの一部のみです。全件取得・購入済みの判定は未検証です。');
  } finally {
